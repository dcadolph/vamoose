...
`vamoose app` opens a local dashboard: run workflows, act on watched holds, edit workflow JSON, manage schedules, and read the run history. On Windows and Linux, `vamoose tray` puts the moose in the system tray to act on holds from a dropdown and keep the daemon running for you. On macOS, `make tray` builds the native menu bar version.
