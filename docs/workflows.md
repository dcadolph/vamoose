...
Drop a JSON file in `~/.config/vamoose/workflows/<name>.json`, pipe it to `vamoose workflows add` (which validates it first), or author it in the `vamoose app` dashboard's editor. A user file overrides a built-in of the same name. Then run `vamoose run <name>`.
